...
export default function BlueprintMapper({ activeFloorPlan, fetchFloorPlan }) {
  ...
  useEffect(() => {
    if (activeFloorPlan && activeFloorPlan.imageUrl) {
      ...
      setSvgContent(activeFloorPlan.rawSvgContent || null);
  ...
  const handleResetAll = async () => {
    if (window.confirm("Are you sure you want to clear the blueprint? This will deactivate the current layout.")) {
      // Just reset the file inputs
      setBlueprintUrl(null);
      setSvgContent(null);
      setPoints([]);
      setZones([]);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Blueprint successfully uploaded to Cloudinary & active floor plan layout initialized!');
      } else {
  ...
  const saveZonesToDb = async (updatedZones) => {
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(`Failed to save mapping to DB: ${data.error}`);
      }
    } catch (err) {
      console.error(err);
      alert('Failed to connect to the backend server to save mapping.');
    }
  };
