...
export default function FloorPlanViewer({
  ...
  // Helper to enable smooth panning transition during selections
  ...
  useEffect(() => {
    if (activeStall && containerRef.current) {
      ...
      const targetScale = 1.6; // Zoom level for details zoom
  ...
  const handleZoom = (factor) => {
    ...
    // Maintain focus point under zoom
    ...
    <div className="relative w-full h-full min-h-[450px] md:min-h-[550px] bg-slate-950 border border-slate-800/80 rounded-xl overflow-hidden shadow-2xl flex flex-col">
      ...
      <div className="absolute bottom-4 left-4 z-20 glass-panel p-3 rounded-lg hidden md:block select-none pointer-events-none">
        ...
        <div className="flex flex-col gap-1.5 text-xs text-slate-300">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Main Keynote Stage</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3.5 h-3.5 bg-emerald-500/10 border border-emerald-500/40 rounded"></span>
            <span>Refreshment Zones</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3.5 h-3.5 bg-slate-900 border border-red-500/50 rounded flex items-center justify-center text-[8px] text-red-500 font-bold">+</span>
            <span>Safety Station / Aid</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {/* Exhibition Outer Hall Border Walls */}
          <rect
            x="10"
            y="10"
            width={svgWidth - 20}
          ...
          {/* Non-Stall Major Zones (Stage, Food court, Restrooms) */}
          {nonStallZones.map((zone) => {
          ...
          {stalls.map((stall) => {
            ...
            return (
              <g key={stall.id} className="group">
                ...
                {/* Stall Host / Exhibitor name (shortened) */}
                <text
                  x={stall.center.x}
                  y={stall.center.y + 10}
                  textAnchor="middle"
          ...
          {landmarks.map((landmark) => (
            <g
              ...
              onClick={() => {
                // Find associated main zone if any
            ...
            >
              ...
              {/* Landmark Tooltip label on hover */}
